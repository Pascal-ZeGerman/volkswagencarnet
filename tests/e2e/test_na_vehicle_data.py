...
async def first_vehicle(na_connection):
    ...
    if not vehicles:
        pytest.fail("No vehicles returned after conn.update() — cannot test vehicle data")
    v = vehicles[0]
...
class TestNAVehicleData:
    ...
    async def test_vehicle_climate_properties(self, first_vehicle):
        ...
        if first_vehicle.is_climatisation_supported:
            ...
            _log.info("Climatisation not supported on this vehicle — climate properties skipped")
        if failures:
            raise AssertionError(
