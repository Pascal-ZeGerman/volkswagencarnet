...
class TestNAPosition:
    ...
    async def test_position_is_not_none(self, first_vehicle):
        ...
        assert pos is not None, (
            "vehicle.position returned None — RVS location endpoint may have failed"
        )
        if pos.get("lat") is None:
            pytest.skip(
                "vehicle.position['lat'] is None — VW_TEST_SPIN may not be set or RVS failed"
            )
        assert pos.get("lng") is not None, "vehicle.position['lng'] is None"
        _log.info(
            "vehicle.position: lat=%s, lng=%s, timestamp=%s",
            pos["lat"],
            pos["lng"],
            pos.get("timestamp"),
        )
    ...
    async def test_position_coordinate_ranges(self, first_vehicle):
        ...
        assert isinstance(pos["lat"], float), (
            f"position['lat'] is not a float: {type(pos['lat']).__name__!r}"
        )
        assert isinstance(pos["lng"], float), (
            f"position['lng'] is not a float: {type(pos['lng']).__name__!r}"
        )
        assert -90.0 <= pos["lat"] <= 90.0, (
            f"latitude {pos['lat']} outside valid range [-90, 90]"
        )
        assert -180.0 <= pos["lng"] <= 180.0, (
            f"longitude {pos['lng']} outside valid range [-180, 180]"
        )
    ...
    async def test_position_timestamp_freshness(self, first_vehicle):
        ...
        assert ts_str is not None, (
            "vehicle.position['timestamp'] is None — RVS may not include timestamp"
        )
        try:
    ...
    async def test_raw_rvs_position_field_and_freshness(self, first_vehicle):
        ...
        if na_loc is None:
            pytest.skip(
                "na_location not in vehicle._states — VW_TEST_SPIN may not be set"
            )
        assert na_loc.get("location") is not None, (
            "na_location has no 'location' sub-dict"
        )
        ...
        if ts_str is not None:
            ...
            age_days = (datetime.now(timezone.utc) - ts).days
            assert age_days <= 7, (
                f"RVS location timestamp {ts_str!r} is older than 7 days"
            )
        else:
            _log.warning(
                "na_location has no 'eventTimeStamp' field — skipping freshness check"
            )
